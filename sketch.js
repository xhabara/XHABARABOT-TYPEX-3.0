...
function setup() {
  ...
  refreshButton = createButton("REFRESH")
    .position(850, 460) // Adjust position as needed
  ...
  if (globalRateSlider.elt) {
    globalRateSlider.elt.style.background =
      "linear-gradient(90deg, #FF5722 0%, #4CAF50 100%)";

    includeRateCheckbox = createCheckbox(
      " Include Autonomous Rate in Xhabarabot Mode",
      false
    )
      .position(430, 400)
      .style("color", "#423F3F")
...
function toggleAutoSlide() {
  ...
  if (autoSlideActive) {
    ...
    if (autoSlideTimeoutSlider.elt) {
      autoSlideTimeoutSlider.elt.style.background =
        "linear-gradient(90deg, #FF5722 0%, #4CAF50 100%)";
    }
...
function startAutoSlide() {
  ...
  autoSlideTimeout = autoSlideTimeoutSlider.value();
...
function setupSlider() {
  ...
  if (delaySlider.elt) {
    ...
    delaySlider.elt.style.background =
      "linear-gradient(90deg, #FF5722 0%, #4CAF50 100%)";
...
function keyPressed() {
  if (!accessGranted) {
    console.log("Access denied. Enter the correct password to play.");
    return;
  }
...
function toggleSync() {
  ...
  if (syncActive) {
    ...
    syncButton.style("background-color", "#4CAF50");
    sharedDelayTime = delaySlider.value();
    syncInputs();
  } else {
    syncButton.html("SYNC");
    syncButton.style("background-color", "#FF5722");
  }
}
...
function toggleRecording() {
  ...
  if (isRecording) {
    ...
    soundRecorder.setInput(compressor);
    soundRecorder.record(recordedSound);
  } else {
    saveButton.html("Start Recording");
    soundRecorder.stop();
    setTimeout(() => {
      recordedSound.save("XhabarabotTypex3_Sequence.wav");
    }, 1000);
  }
}
